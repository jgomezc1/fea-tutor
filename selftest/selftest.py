...
STUDENT_RESPONSES = {
    ...
    "local_global_dofs": {
        "correct_deep": (
            "The IBC array maps each node's DOF to an equation number in "
            "the reduced system. Fixed nodes get -1 (they're not unknowns), "
            "and free nodes get sequential equation numbers starting from 0. "
            "This is how boundary conditions are applied — by excluding "
            "constrained DOFs from the system of equations rather than "
            "modifying the stiffness matrix. For a 4-node system with "
            "node 0 fixed: IBC = [-1, 0, 1, 2], giving us 3 equations "
            "for the 3 unknown displacements."
        ),
        "correct_mechanical": (
            "IBC assigns -1 to fixed nodes and 0, 1, 2... to free nodes."
        ),
    },
    "assembly": {
        "correct_deep": (
...
def scenario_quick():
    ...
    return {
        ...
        "steps": [
            ...
            {"action": "evaluate", "node": "local_global_dofs", "state": None,
             "response_key": "correct_deep"},
